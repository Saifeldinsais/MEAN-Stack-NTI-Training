...
const addTaskToFav = async (req, res) => {
  try {
    ...
    if (!user.favTasks.includes(taskID)) {
      user.favTasks.push(taskID);
      await user.save();
    }

    res.status(200).json({ status: "success", data: { favTasks: user.favTasks } });
  } catch (error) {
...
module.exports = { signup, getAllUsers, login, protectRoutes, addTaskToFav };